...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/backtest_1m.log"),
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger("MLBacktest_1m")
...
async def run_1m_backtest():
    ...
    # Calculate best_bid/ask for MockExchange (Simple simulation)
    ...
    # Override Strategy Params (Safe Settings for 1m test)
    strategy.max_loss_usd = 5000.0  # Increased for 1 month volume
    strategy.max_drawdown_pct = 0.40
    strategy.grid_layers = 5 
    strategy.order_size_usd = 100.0

    # Mock Prediction Logic
    def mock_predict_live_proba(symbol="BTCUSDT"):
        # Use strategy.candles which we inject below
    ...
    # 3. Running Backtest
    # LIMIT TO 1 MONTH (30 days * 24h * 60m = 43200)
    steps = min(len(df), 43200) 
    equity_history = []
    
    print(f"Starting 1m Backtest for {steps} minutes (1 Month)...")
    ...
    for i in range(start_idx, steps - 1):
        ...
        # Inject recent candles for ML (Rolling window of 50m)
        # Using 1m candles for prediction
        window_start = max(0, current_idx - 60) # 60 minutes window for features
    ...
    print("\n" + "="*30)
    print("1-MINUTE BACKTEST RESULTS")
    print("="*30)
