"""
Parameter Sweep: price_tolerance & grid_spacing — 1-MINUTE DATA
----------------------------------------------------------------
More realistic than hourly: captures order re-placement dynamics
that happen during steady price trends.
"""
import asyncio
import pandas as pd
import numpy as np
...
logger = logging.getLogger("Sweep1m")
...
def convert_1m(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Use 'timestamp' column directly (already in ms)
...
async def main():
    ...
    # Use last 10,000 candles (~7 days) for speed
    CANDLES = 10000
    df = df.tail(CANDLES).reset_index(drop=True)
    df = convert_1m(df)
    logger.info(f"Using {len(df)} 1-minute candles ({df['timestamp'].iloc[0]} to {df['timestamp'].iloc[-1]})")
    
    # Parameter grid
    tolerances = [0.001, 0.002, 0.003, 0.005, 0.008, 0.01]
    spacings = [0.0010, 0.0015, 0.0020, 0.0030]
    ...
    for i, (tol, spc) in enumerate(combos):
        ...
        if result:
            ...
            logger.info(
                f"[{i+1}/{len(combos)}] tol={tol*100:.2f}% spc={spc*100:.2f}% | "
                f"PnL=${result['pnl']:+.2f} ({result['pnl_pct']:+.1f}%) | "
                f"Fills={result['fills']:4d} | Cancels={result['cancel_cycles']:5d} | "
                f"F/C={result['fills_per_cancel']:.4f} | "
    ...
    logger.info("\n" + "=" * 110)
    logger.info("1-MINUTE BACKTEST RESULTS — RANKED BY P&L")
    logger.info("=" * 110)
    logger.info(f"{'Rank':>4} | {'Tol%':>6} | {'Spc%':>6} | {'PnL$':>10} | {'PnL%':>7} | {'Sharpe':>7} | {'MaxDD%':>7} | {'Fills':>6} | {'Cancels':>8} | {'Fill/Cancel':>11}")
    logger.info("-" * 110)
    
    for i, r in enumerate(results):
        marker = " ◀" if r['tolerance'] == 0.005 and r['spacing'] == 0.002 else ""
        logger.info(
            ...
            f"{r['fills_per_cancel']:>11.4f}{marker}"
        )
    ...
    best_efficiency = max(results, key=lambda r: r['fills_per_cancel'])
    least_cancels = min(results, key=lambda r: r['cancel_cycles'])
    
    logger.info(f"🏆 Best PnL:       tol={best_pnl['tolerance']*100:.2f}% spc={best_pnl['spacing']*100:.2f}% → ${best_pnl['pnl']:+.2f} ({best_pnl['fills']} fills)")
    logger.info(f"📈 Best Sharpe:    tol={best_sharpe['tolerance']*100:.2f}% spc={best_sharpe['spacing']*100:.2f}% → {best_sharpe['sharpe']:.2f}")
    logger.info(f"🔥 Most Fills:     tol={best_fills['tolerance']*100:.2f}% spc={best_fills['spacing']*100:.2f}% → {best_fills['fills']} fills")
    logger.info(f"⚡ Best F/C Ratio: tol={best_efficiency['tolerance']*100:.2f}% spc={best_efficiency['spacing']*100:.2f}% → {best_efficiency['fills_per_cancel']:.4f}")
    logger.info(f"🔇 Least Cancels:  tol={least_cancels['tolerance']*100:.2f}% spc={least_cancels['spacing']*100:.2f}% → {least_cancels['cancel_cycles']} cancels")
    
    results_df = pd.DataFrame(results)
    out_file = "data/sweep_tolerance_1m_results.csv"
